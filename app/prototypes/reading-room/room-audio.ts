...
/** Call unlock from a user gesture; construction and preferences are SSR safe. */
export function createRoomAudio(): RoomAudio {
  ...
  function loadImpacts(): Promise<void> {
    ...
    impactsLoading = Promise.allSettled(
      missing.map(async (url) => {
        const response = await fetch(url, { signal: audioRequest.signal })
        ...
        if (!disposed && graph === currentGraph)
          currentGraph.buffers.set(url, buffer)
      })
    ).then(() => {
      // A failed take doesn't disable the other recordings. Later gestures retry
      // only missing takes; collisions never fetch, decode, or replay stale hits.
  ...
  async function loadMusic() {
    ...
    musicLoading = true
    try {
      const response = await fetch(MUSIC_URL, { signal: audioRequest.signal })
      if (!response.ok) throw new Error('Background audio unavailable')
      const encoded = await response.arrayBuffer()
      if (disposed) return
      // Decode at the asset's sample rate instead of expanding five minutes
      // to the output device's rate. The audio graph resamples during playback.
      const decoder = new OfflineAudioContext(2, 1, MUSIC_SAMPLE_RATE)
      const buffer = await decoder.decodeAudioData(encoded)
      if (disposed || graph !== currentGraph) return
      ...
      // Sound effects remain available; a later gesture can retry the music.
    } finally {
      musicLoading = false
    }
  }
  ...
  return {
    async unlock() {
      ...
      try {
        ...
        // A blocked or interrupted context can be unlocked by the next gesture.
        return
      }
      if (disposed || currentGraph !== graph) return
      setLevel(
